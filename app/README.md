...
macOS用のビルド成果物は、ソース識別情報と対象OS/CPUが同じ場合にGitHub Actions Cacheから
検証して再利用します。
...
- 複数ビデオトラックの選択とプレビューへの即時反映
- 複数音声トラックの選択、プレビューでの同期ミックス、トラック別ゲイン調整、1トラックへのミックス
- 再生中のトラック別リアルタイム音量メーター（dBFS）
- 音声トラックの全選択・全解除、ソロモニター、リアルタイム音量波形
- 平均音量を解析しておすすめゲインを算出・適用する音量ミキサー
- 自動調整時の解析範囲確認と、トラックごとの解析進行率表示
