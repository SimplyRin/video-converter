...
そのブランチを次のパッチバージョンのスナップショットとしてビルド・公開します。リポジトリの最新の
安定版タグのパッチ番号を1つ進め、`calculate-build-number.sh`の`BUILDNUM`を使って
`vMAJOR.MINOR.NEXTFIX-SNAPSHOT_BUILDNUM`形式のGitタグを作成します。

例えば、最新の安定版タグが`v1.0.9`、`BUILDNUM`が`4091`の場合、GitHub Releaseのタグ、
配布ファイル名、アプリ本体の完全バージョンは`v1.0.10-SNAPSHOT_4091`となり、Pre-releaseとして
公開されます。同じ`v1.0.10`の正式版が公開された場合、スナップショット版から更新できます。
